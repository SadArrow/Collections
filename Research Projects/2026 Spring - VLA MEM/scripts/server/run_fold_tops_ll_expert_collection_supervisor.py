...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Collect Fold Tops low-level expert data with one isolated Isaac process per attempt "
            "to avoid scene/object residue across attempts."
        )
    )
...
def _run_child_attempt(
    *,
    ...
    command = [
        ...
        "--script_rel",
        "../myVLA/scripts/data/collect_fold_tops_ll_expert_dataset.py",
        "--run_as_main",
        "--",
        "--dex_root",
        os.fspath(Path(args.dex_root).expanduser().resolve()),
...
def main() -> int:
    ...
    while successes < int(args.target_successes) and attempts < int(args.max_attempts):
        ...
        print(
            json.dumps(
                {
                    "event": "supervisor_attempt_start",
        ...
        child_summary = _read_json(child_output_dir / "summary.json") if (child_output_dir / "summary.json").is_file() else {}
        ...
        if success:
            ...
            aggregate_record["success_index"] = int(successes)
        ...
        print(
            json.dumps(
                {
                    "event": "supervisor_attempt_finish",
                    "attempt": int(attempts),
                    "success": bool(success),
                    "successes": int(successes),
                    "launcher_return_code": int(return_code),
