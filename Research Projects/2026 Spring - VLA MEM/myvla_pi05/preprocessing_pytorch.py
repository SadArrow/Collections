...
logger = logging.getLogger("myvla_pi05")
...
def preprocess_observation_pytorch(
    ...
    *,
    ...
    for key in image_keys:
        ...
        # Handle both single-image and video tensors, in either channels-first or channels-last formats.
        if is_video:
            channels_first = image.shape[2] == 3
            channels_last = image.shape[-1] == 3
            if channels_first and channels_last:
                # Ambiguous but rare; prefer channels-first.
                channels_last = False
            if channels_first:
                image = image.permute(0, 1, 3, 4, 2)  # [B, T, C, H, W] -> [B, T, H, W, C]
            elif not channels_last:
                raise ValueError(f"Unexpected video image shape for {key}: {tuple(image.shape)}")

            batch, time = image.shape[:2]
            image = image.reshape(batch * time, *image.shape[2:])  # [B*T, H, W, C]
        else:
        ...
        if is_video:
            image = image.reshape(batch, time, *image.shape[1:])  # [B, T, H, W, C]
            if channels_first:
                image = image.permute(0, 1, 4, 2, 3)  # [B, T, H, W, C] -> [B, T, C, H, W]
        else:
