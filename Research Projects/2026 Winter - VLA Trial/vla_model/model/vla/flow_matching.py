"""
Flow Matching for action chunk generation.
Based on GraspVLA-main/vla_network/model/vla/flow_matching.py
"""
from typing import Optional, Tuple, Union, Callable
import torch
from torch import nn

from vla_model.config import FlowMatchingConfig
...
class BaseFlowMatchingModule(nn.Module):
    ...
    def get_time_embedding(self, t: torch.Tensor) -> torch.Tensor:
        return posemb_sincos(t, self.embed_dim)

    def sample_time(
        self, batch_shape: tuple, device: torch.device, dtype: torch.dtype
    ) -> torch.FloatTensor:
        ...
        return time * (self.config.time_max - self.config.time_min) + self.config.time_min

    def sample_noise(
        self, shape: tuple, device: torch.device, dtype: torch.dtype
    ) -> torch.FloatTensor:
    ...
    def update(
        ...
        dt: Union[float, torch.FloatTensor],
    ...
    def denoise(
        self,
        compute_v_t: Callable[[torch.FloatTensor, torch.FloatTensor], torch.FloatTensor],
        x_t: torch.FloatTensor,
        iter_num: int,
    ) -> torch.FloatTensor:
...
class VLAFlowMatchingModule(BaseFlowMatchingModule):
    """Flow matching for VLA action generation."""
    ...
    def sample_noise(
        ...
        return super().sample_noise(
            (batch_size, self.action_len, self.action_dim), device=device, dtype=dtype
        )

    def get_suffix_masks(self, proprio_embed: torch.Tensor):
        ...
        block_mask[0] = True
        block_mask[proprio_embed.shape[1]] = True
    ...
    def embed_suffix_flow_matching_embeds(
        ...
        return torch.cat([proprio_embed, action_time_embeds], dim=1)

    def get_v_t(self, hidden_states: torch.FloatTensor) -> torch.FloatTensor:
